...
namespace CoffeeLands.Models
{
    public sealed class PaypalClient
    {
        ...
        public async Task<CreateOrderResponse> CreateOrder(string value, string currency, string reference)
        {
            ...
            //public async Task<CreateOrderResponse> CreateOrder(OrderProduct order)
            //{
            //    var auth = await Authenticate();
            ...
            //                {
            //                    currency_code = "USD",
            //            value = order.Grand_total.ToString()
            //                }
            //            }
            //        }
            //    };
		...
		//public async Task<CreateOrderResponse> CreateAndCaptureOrder(OrderProduct order)
		//{
		//	var createOrderResponse = await CreateOrder(order);

		//	// Kiểm tra trạng thái tạo đơn hàng thành công
		//	if (createOrderResponse != null && createOrderResponse.status == "CREATED")
		//	{
		//		// Capture đơn hàng ngay sau khi tạo (chỉ áp dụng cho môi trường sandbox, bạn có thể cần xử lý khác cho môi trường live)
		//		var captureOrderResponse = await CaptureOrder(createOrderResponse.id);

		//		// Kiểm tra trạng thái capture
		//		if (captureOrderResponse != null && captureOrderResponse.status == "COMPLETED")
		//		{
		//			// Đã capture đơn hàng thành công, có thể thực hiện các bước xử lý khác
		//			return createOrderResponse;
		//		}
		//	}

		//	// Xử lý trường hợp tạo hoặc capture đơn hàng không thành công
		//	return null;
		//}
